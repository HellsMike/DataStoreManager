...
namespace OrderService.Service
{
    public class IncomingService : IIncomingService
    {
        ...
        private readonly IIncomingRepository _repository;
        private readonly IEventRepository _eventRepository;

        public IncomingService(ILogger<IncomingService> logger, IIncomingRepository orderRepository, IEventRepository eventRepository)
        {
            _logger = logger;
        ...
        public async Task AddUpdate(OrderMessage orderMsg)
        {
            ...
            var toUpdate = await _repository.GetSingleOrder(x => x.Type.Contains("I") && x.Number.Equals(orderMsg.number));
            if (toUpdate is null)
            {
                var toCreate = await Order.Create(orderMsg);
                await _repository.Add(toCreate);
                var message = await OrderEvent.Create(toCreate, "In_AddOrder");
            ...
            else if (toUpdate.Status == Order.OrderStatus.New || toUpdate.Status == Order.OrderStatus.Suspended)
            {
                ...
                var message = await OrderEvent.Create(toUpdate, "In_AddPalletToOrder");
                await _eventRepository.Add(message);
            }
            else
        ...
        private async Task PalletsUpdate(Order toUpdate, OrderMessage orderMsg, Order.OrderStatus status)
        {
            ...
            foreach (Pallet tempPallet in temp.Pallets)
            {
                ...
                {
                    if (tempPallet.Lpn.Equals(realPallet.Lpn))
                    {
                        found = true;
                        break;
                    }
        ...
        private async Task CheckPalletsReserved(OrderMessage orderMsg)
        {
            foreach (var palletMsg in orderMsg.pallets)
            {
                ...
                var orders = await _repository.GetOrders(ord => ord.Type.Contains("I") && ord.Pallets.Any(plt => plt.Lpn.Equals(pallet.Lpn)));
                if (orders.Any())
                {
        ...
        {
            if (page is null)
                return await _repository.GetOrders(x => x.Type.Contains("I"));
            if (page > 0)
        ...
        public async Task RemovePallet(OrderMessage orderMsg)
        {
            ...
            else if (toUpdate.Status == Order.OrderStatus.New || toUpdate.Status == Order.OrderStatus.Suspended)
            {
                foreach (var palletMsg in orderMsg.pallets)
                {
                    try
                    {
                        var error = !toUpdate.Pallets.Remove(toUpdate.Pallets.Where(x => x.Lpn.Contains(palletMsg.lpn)).First());
                        if (error)
                ...
                await _repository.Update(toUpdate);
                var message = await OrderEvent.Create(toUpdate, "In_RemovePalletFromOrder");
                await _eventRepository.Add(message);
            }
            else
        ...
        public async Task ChangeStatus(string orderNumber, Order.OrderStatus status)
        {
            ...
            var toUpdate = await _repository.GetSingleOrder(x => x.Type.Contains("I") && x.Number.Equals(orderNumber));

            switch (status)
            {
                case Order.OrderStatus.Active:
                    await CheckPalletReserved(toUpdate);
                    statusString = "In_ActiveOrder";
                    break;
                case Order.OrderStatus.Suspended:
                    statusString = "In_SuspendOrder";
                    break;
                case Order.OrderStatus.Closed:
                    statusString = "In_CloseOrder";
                    break;
                case Order.OrderStatus.Deleted:
                    statusString = "In_DeleteOrder";
                    break;
            }
            ...
            switch (toUpdate.Status)
            {
                    ...
                    if (status == Order.OrderStatus.Closed)
                    {
                        foreach (var pallet in toUpdate.Pallets)
                        {
                            if (pallet.Status != Pallet.PalletStatus.StoredOrShipped)
                                throw new InvalidOperationException("You can't close an order before having stored its pallets");
                        }
                    }
                    break;
                ...
                case Order.OrderStatus.Suspended:
                    ...
                    break;
                case Order.OrderStatus.Closed:
                    if (status == Order.OrderStatus.Active || status == Order.OrderStatus.Suspended)
                        throw new InvalidOperationException("A closed order can't be set to Active or Suspended");
                    break;
                case Order.OrderStatus.Deleted:
        ...
        public async Task StorePallet(string orderNumber, string lpn)
        {
            var order = await _repository.GetSingleOrder(x => x.Type.Contains("I") && x.Number.Equals(orderNumber));
            if (order is null)
                throw new ArgumentException($"Order #{orderNumber} doesn't exist", nameof(orderNumber));
            var orderList = await _repository.GetOrders(ord => ord.Type.Contains("I") && ord.Pallets.Any(plt => plt.Lpn.Equals(lpn)));
            var found = false;
            ...
            var message = await OrderEvent.Create(order, "In_StorePallet");
            await _eventRepository.Add(message);
        }
    }
}
